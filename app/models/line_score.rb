class LineScore < ActiveRecord::Base
  ...
  def self.get
    Team.find_each do |team|
      ...
      if num == '2'
        LineScore.seed("#{year}_#{month}_#{day}_#{away_team}_#{home_team}_1")
      end
      ...
      begin
        ...
        line.each do |line|
          @score = {
            ...
            wp_fname: wp.attribute('first').text,
            wp_lname: wp.attribute('last').text,
            lp_fname: lp.attribute('first').text,
            lp_lname: lp.attribute('last').text,
            sv_fname: sv.attribute('first').text,
            sv_lname: sv.attribute('last').text,
